...
''' insert by KS'''
from CPAC.func_preproc.calib_preproc import create_calib_preproc
from CPAC.func_preproc.bo_func_preproc import create_bo_func_preproc
from CPAC.registration.registration_via_synth import create_linear_func_to_synth, create_linear_rho_to_anat, create_bbr_func_to_anat_via_synth, create_bbregister_func_to_anat, create_ants_nonlinear_xfm, create_apply_ants_xfm, create_register_func_to_mni
''' end '''

from CPAC.seg_preproc.seg_preproc import create_seg_preproc

#from CPAC.registration.registration_via_synth import create_nonlinear_register, create_register_func_to_anat, create_bbregister_func_to_anat, \
...
#    strat_list += new_strat_list
    ...
    workflow_counter += 1
    if 1 in c.runFunctionalPreprocessingWithCalib:
        
	# first preprocess calibration scans
	calib_preproc = create_calib_preproc(wf_name='calib_preproc_%d' % num_strat)
	calib_preproc.inputs.inputspec.bo_name = sub_dict['calib']['cal_bo']
	calib_preproc.inputs.inputspec.reg_bo_name = sub_dict['calib']['cal_reg_bo']
	calib_preproc.inputs.inputspec.rho_name = sub_dict['calib']['cal_rho']
	calib_preproc.inputs.inputspec.rs_name = sub_dict['calib']['cal_rs']
	
	
	
    
        ''' KS - Q: should this be func_preproc or bo_func_preproc'''
        workflow_bit_id['func_preproc'] = workflow_counter
        for strat in strat_list:
            ...
            if slice_timing:
                ...
                bo_func_preproc = create_bo_func_preproc(slice_timing_correction=True, wf_name='bo_func_preproc_%d' % num_strat)
		...
		#pass the calib reg bo RPI for unwarping
                workflow.connect(calib_preproc, 'outputspec.cal_reg_bo_RPI',
                                 bo_func_preproc, 'inputspec.calib_reg_bo_RPI')
		
		
		# passing the slice timing correction parameters
		workflow.connect(scan_params, 'tr',
                                 bo_func_preproc, 'scan_params.tr')
                workflow.connect(scan_params, 'ref_slice',
                                 bo_func_preproc, 'scan_params.ref_slice')
                workflow.connect(scan_params, 'tpattern',
                                 bo_func_preproc, 'scan_params.acquisition')

                workflow.connect(scan_params, 'start_indx',
                                 bo_func_preproc, 'inputspec.start_idx')
                workflow.connect(scan_params, 'stop_indx',
                                 bo_func_preproc, 'inputspec.stop_idx')

                workflow.connect(scan_params, 'tr',
                                 convert_tr, 'tr')
            else:
                bo_func_preproc = create_bo_func_preproc(slice_timing_correction=False, wf_name='bo_func_preproc_%d' % num_strat)
                bo_func_preproc.inputs.inputspec.start_idx = c.startIdx
                bo_func_preproc.inputs.inputspec.stop_idx = c.stopIdx
		workflow.connect(calib_preproc, 'outputspec.cal_reg_bo_RPI',
                                 bo_func_preproc, 'inputspec.calib_reg_bo_RPI')
            ...
            try:
                node, out_file = strat.get_leaf_properties()
                workflow.connect(node, out_file, bo_func_preproc, 'inputspec.rest')
            ...
            if 0 in c.runFunctionalPreprocessingWithCalib:
                # we are forking so create a new node
                tmp = strategy()
                tmp.resource_pool = dict(strat.resource_pool)
                tmp.leaf_node = (strat.leaf_node)
            ...
            strat.append_name(bo_func_preproc.name)

            strat.set_leaf_properties(bo_func_preproc, 'outputspec.preprocessed')

            # add stuff to resource pool if we need it
            if slice_timing:
                strat.update_resource_pool({'slice_timing_corrected': (bo_func_preproc, 'outputspec.slice_time_corrected')})
            strat.update_resource_pool({'mean_functional':(bo_func_preproc, 'outputspec.example_func')})
            strat.update_resource_pool({'functional_preprocessed_mask':(bo_func_preproc, 'outputspec.preprocessed_mask')})
            strat.update_resource_pool({'movement_parameters':(bo_func_preproc, 'outputspec.movement_parameters')})
            strat.update_resource_pool({'max_displacement':(bo_func_preproc, 'outputspec.max_displacement')})
            strat.update_resource_pool({'preprocessed':(bo_func_preproc, 'outputspec.preprocessed')})
            strat.update_resource_pool({'functional_brain_mask':(bo_func_preproc, 'outputspec.mask')})
            strat.update_resource_pool({'motion_correct':(bo_func_preproc, 'outputspec.motion_correct')})
	    strat.update_resource_pool({'functional_bo_unwarp':(bo_func_preproc, 'outputspec.bo_unwarped')})
	    
	    # update calibration essentials to resource pool to be used later
	    strat.update_resource_pool({'calib_bo_RPI':(calib_preproc, 'outputspec.cal_bo_RPI')})
            strat.update_resource_pool({'calib_reg_bo_RPI':(calib_preproc, 'outputspec.cal_reg_bo_RPI')})
            strat.update_resource_pool({'calib_rho_RPI':(calib_preproc, 'outputspec.cal_rho_RPI')})
	    strat.update_resource_pool({'calib_rs_RPI':(calib_preproc, 'outputspec.cal_rs_RPI')})
	    strat.update_resource_pool({'calib_synth_RPI':(calib_preproc, 'outputspec.cal_synth_RPI')})
	    
	    
	    
	    create_log_node(bo_func_preproc, 'outputspec.preprocessed', num_strat)
            num_strat += 1
...
#    """
#    Func -> T1 Registration (Initial Linear reg) VIA CALIBRATION
#    """
#
#    # Depending on configuration, either passes output matrix to Func -> Template ApplyWarp,
#    # or feeds into linear reg of BBReg operation (if BBReg is enabled)
...
#    workflow_counter += 1
#    
#    if 1 in c.runRegisterFuncToAnatWithCalib:
#        
#	''' KS Q: func_to_anat OR func_to_anat_via_calib???'''
#        workflow_bit_id['func_to_anat'] = workflow_counter
#        for strat in strat_list:	
#	    func_to_synth = create_linear_func_to_synth('func_to_synth_FLIRT_%d' % num_strat)
#	    
#	    workflow.connect(funcFlow, 'outputspec.rest',func_to_synth, 'inputspec.func' )
#	    
#	    node, out_file = strat.get_node_from_resource_pool('calib_synth_RPI')
#	    workflow.connect(node, out_file, func_to_synth, 'inputspec.synth_RPI')
#	    
#	    node, out_file = strat.get_node_from_resource_pool('calib_bo_RPI')
#	    workflow.connect(node, out_file, func_to_synth, 'inputspec.calib_bo_RPI')
#	    
#	    rho_to_anat
#	    
#	    
...
#    strat_list += new_strat_list
    ...
    if 1 in c.runBBRegWithCalib:
        ''' KS Q: func_to_anat_bbreg OR func_to_anat_with_calib_bbreg both in bit_id and the functions'''
	
        workflow_bit_id['func_to_anat_bbreg'] = workflow_counter
        for strat in strat_list:
            func_to_anat_bbreg_with_calib = create_bbr_func_to_anat_via_synth('func_to_anat_bbreg_with_calib_%d' % num_strat)
       
            # Input registration parameters
            func_to_anat_bbreg_with_calib.inputs.inputspec.bbr_schedule = c.boundaryBasedRegistrationSchedule
            ...
            try:
                ...
                workflow.connect(funcFlow, 'outputspec.rest',
                                 func_to_anat_bbreg_with_calib, 'inputspec.func')
		
		
		node, out_file = strat.get_node_from_resource_pool('calib_synth_RPI')
	    	workflow.connect(node, out_file, func_to_anat_bbreg_with_calib, 'inputspec.synth_RPI')
	    
	    	node, out_file = strat.get_node_from_resource_pool('calib_bo_RPI')
	    	workflow.connect(node, out_file, func_to_anat_bbreg_with_calib, 'inputspec.calib_bo_RPI')
              
	        node, out_file = strat.get_node_from_resource_pool('calib_rho_RPI')
	    	workflow.connect(node, out_file, func_to_anat_bbreg_with_calib, 'inputspec.calib_rho_RPI')
		...
		# Input segmentation probability maps for white matter segmentation
                ...
                workflow.connect(node, (out_file, pick_wm),
                                 func_to_anat_bbreg_with_calib, 'inputspec.anat_wm_segmentation')

                # Input anatomical whole-head image (reoriented)
                node, out_file = strat.get_node_from_resource_pool('anatomical_reorient')
                workflow.connect(node, out_file,
                                 func_to_anat_bbreg_with_calib, 'inputspec.anat_skull')
...
#                                 func_to_anat_bbreg, 'inputspec.linear_reg_matrix')
            ...
            strat.append_name(func_to_anat_bbreg_with_calib.name)
            # strat.set_leaf_properties(func_mni_warp, 'out_file')

            strat.update_resource_pool({'mean_functional_in_anat':(func_to_anat_bbreg_with_calib, 'outputspec.anat_func_via_synth'),
                                        'functional_to_anat_linear_xfm':(func_to_anat_bbreg_with_calib, 'outputspec.func_to_anat_via_synth_linear_xfm')})
